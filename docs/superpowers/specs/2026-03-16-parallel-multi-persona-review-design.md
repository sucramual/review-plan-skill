...
- **Duplicate personas** are deduplicated before dispatch.
...
1. Build the full reviewer prompt for **each persona independently** — same process as today: persona instruction + context materials + full plan content + review template from `references/reviewer-prompt.md`. Only the `{PERSONA_INSTRUCTION}` differs between prompts.
...
**Codex backend (`--reviewer codex`):** Each persona gets its own Codex process with a unique temp file (e.g., `/tmp/review-plan-output-security-engineer.md`). All temp files cleaned up after synthesis.
...
2. **Deduplicate** — identify issues that overlap across personas, even if framed differently (e.g., "auth gap" from a security engineer and "missing component" from an architect are the same underlying issue).
...
- `[N/M personas]` or `[N/M — persona names]` tags on each issue.
